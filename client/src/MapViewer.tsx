...
export class MapViewer extends Component<MapProps, MapState> {
   ...
   */
  doGetEndpointsAtClick = (hour: Hour): [Building, Building] => {
  ...
  renderEndPoints = (): Array<JSX.Element> => {
    ...
    const [start, end] = this.doGetEndpointsAtClick(this.state.hour);
    ...
    if (this.state.nearby) {
      for (const [index, friend] of this.state.nearby.entries()) {
        elems.push(
          <circle
            cx={friend.loc.x}
            cy={friend.loc.y}
            r={RADIUS}
            fill={FRIEND_COLORS[index % FRIEND_COLORS.length]}
            stroke={'white'}
            strokeWidth={10}
            key={'friend - ' + index}
          />
  ...
  doRenderPathClick = (): Array<JSX.Element> => {
  ...
  renderLegendItems = (): Array<JSX.Element> => {
    ...
    if (this.state.nearby) {
      for (const [index, friend] of this.state.nearby.entries()) {
        const color: string = FRIEND_COLORS[index % FRIEND_COLORS.length];
        items.push(makeLegendItem(color, `${friend.friend} is nearby`, `${friend.friend}`));
      }
    }
    return items;
  };
