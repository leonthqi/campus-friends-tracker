...
import { Friends, parseFriends } from './friends';
...
export class App extends Component<AppProps, AppState> {
    ...
    if (!this.state.buildings || (this.state.user && !this.state.schedule)) {
      ...
      return <div className="content main">
        <div>
          ...
          <FriendsEditor user={this.state.user}
          friends={friends}
          onFriendClick={this.doFriendsChange}
        />
          }
        </div>
  ...
  doFriendsChange = (newFriends: Friends): void => {
    this.setState({ friends: newFriends, saved: false });
  
    fetch("/api/setData", {
      method: 'POST',
      body: JSON.stringify({user: this.state.user,friends: Array.from(newFriends)}),
  ...
  doGetDataJson = (data: unknown): void => {
    ...
    this.setState({show: "edit", schedule, savedSchedule: schedule, friends: friends});
  ...
  doBackClick = (): void => {
    this.setState({show: "log-in", user: undefined,
      schedule: undefined, savedSchedule: undefined, friends: undefined, saved: false});
  }
