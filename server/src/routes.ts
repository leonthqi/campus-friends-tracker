...
export const getUserData = (req: SafeRequest, res: SafeResponse): void => {
  ...
  const friendsList = friends === undefined ? [] : Array.from(friends);
...
export const getShortestPath = (req: SafeRequest, res: SafeResponse): void => {
  ...
  if (friends !== undefined) {
    ...
    for (const friend of friends) {
      ...
      if (!allFriends.get(friend)?.includes(user))
       continue;
      ...
      const closestInTree = findClosestInTree(userLocsTree, friendLocs);
      if (closestInTree[0] !== undefined && closestInTree[1] !== Infinity) {
      nearby.push({friend: friend,loc: closestInTree[0],dist: closestInTree[1]});
}
    }
  }
