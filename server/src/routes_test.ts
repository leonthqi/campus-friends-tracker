...
describe('routes', function() {
  ...
  it('getShortestPath', function() {
    ...
    // Set Leon's schedule and mutual friendship
    const req9 = httpMocks.createRequest(
        {method: 'POST', url: '/api/setData',
         body: {user: "Leon", schedule: [
            {hour: "9:30", location: "DEN", desc: "CSE 311 section"},
            {hour: "10:30", location: "SUZ", desc: "ENG 131"},
            {hour: "1:30", location: "KAN", desc: "BIOL 180"},
          ], friends: ["Kevin"]}});
    ...
    // Now Kevin's path at 10:30 should show Leon as nearby
    ...
    assert.deepStrictEqual(res10._getData().nearby.length > 0, true);
    assert.deepStrictEqual(res10._getData().path.length > 0, true);
    assert.deepStrictEqual(res10._getData().nearby[0].dist > 0, true);
    assert.deepStrictEqual(res10._getData().nearby[0].friend, "Leon");
