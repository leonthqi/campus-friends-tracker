...
export const closestInTree =
  ...
  if (distanceMoreThan(loc, bounds, closest.dist) || tree.kind === "empty"){
    return closest;
  } 
  if (tree.kind === "single") {
    const newDist: number = distance(tree.loc, loc);
      if (newDist < closest.dist) {
        return { loc: tree.loc, dist: newDist };
      } else {
        return closest;
      }  
  }
  if (tree.kind === "split") {
    const middle: Location = tree.at;
    const NW: Region = {x1: bounds.x1, x2: middle.x, y1: bounds.y1, y2: middle.y};
    const NE: Region = {x1: middle.x, x2: bounds.x2, y1: bounds.y1, y2: middle.y};
    const SW: Region = {x1: bounds.x1, x2: middle.x, y1: middle.y, y2: bounds.y2};
    const SE: Region = {x1: middle.x, x2: bounds.x2, y1: middle.y, y2: bounds.y2};

    // NW
    if (loc.x < middle.x && loc.y < middle.y) {
      closest = closestInTree(tree.nw, loc, NW, closest);
      if (middle.x - loc.x < middle.y - loc.y) { 
        closest = closestInTree(tree.ne, loc, NE, closest);
        closest = closestInTree(tree.sw, loc, SW, closest);
      } else {
        closest = closestInTree(tree.sw, loc, SW, closest);
        closest = closestInTree(tree.ne, loc, NE, closest);
      }
      closest = closestInTree(tree.se, loc, SE, closest);
    }

    // NE
    else if (loc.x >= middle.x && loc.y < middle.y) {
      closest = closestInTree(tree.ne, loc, NE, closest);
      if (loc.x - middle.x < middle.y - loc.y) {
        closest = closestInTree(tree.nw, loc, NW, closest);
        closest = closestInTree(tree.se, loc, SE, closest);
      } else {
        closest = closestInTree(tree.se, loc, SE, closest);
    ...
    // SW
      ...
      closest = closestInTree(tree.sw, loc, SW, closest);
      if (middle.x - loc.x < loc.y - middle.y) {
        closest = closestInTree(tree.se, loc, SE, closest);
        closest = closestInTree(tree.nw, loc, NW, closest);
      } else {
        closest = closestInTree(tree.nw, loc, NW, closest);
  ...
  // Remove, just here to avoid "declared but never read" error
  return closest;
};